...
class GetInformationFunctions:
    ...
    def search_results(search_keyword):
        rows = cursor.execute("SELECT id, gamertag, ip, xuid, mid FROM userinfo WHERE gamertag LIKE ?", ('%' + search_keyword + '%',))
        for row in cursor:
            print(f"\nID: {row[0]}\nGamertag: {row[1]}\nIP Address: {row[2]}\nXUID: {row[3]}\nMachine ID: {row[4]}\n")
...
class DatabaseFunctions:
    ...
    def merage_database():
        ...
        database_two = input(f"Which database would you like to merge into {database_name}.db: ")
    ...
    # Count how many records exists for gamertags, ips, xuids, and machine IDs (MIDs) and print them.
    # Notes: This doesn't work well at the moment it counts records that are blank.
    def database_stats():
        count_gamertags = cursor.execute("SELECT count(gamertag) FROM userinfo")
        count_gamertags_result = cursor.fetchone()
        count_gamertags_total = count_gamertags_result[0]

        count_ips = cursor.execute("SELECT count(ip) FROM userinfo")
        count_ips_result = cursor.fetchone()
        count_ips_total = count_ips_result[0]

        count_xuids = cursor.execute("SELECT count(xuid) FROM userinfo")
        count_xuids_result = cursor.fetchone()
        count_xuids_total = count_xuids_result[0]

        count_mids = cursor.execute("SELECT count(mid) FROM userinfo")
        count_mids_result = cursor.fetchone()
        count_mids_total = count_mids_result[0]

        print(f"Total Gamertags: {count_gamertags_total}\nTotal IPs: {count_ips_total}\nTotal XUIDs: {count_xuids_total}\nTotal MIDs: {count_mids_total}")
...
class InputFromIPLog:
    ...
    def insert_data_apparitionnet(self):
        ...
        num_duplicates_removed = DatabaseFunctions.remove_duplicates()
...
while valid_input == False:
    ...
    # Delete database. Ask the user the database they want to delete then delete it.
        ...
        valid_input = True
    elif options == "8" or options == "remove":
        GetInformationFunctions.database_files()
        DatabaseFunctions.connect()
        ...
        DatabaseFunctions.close()
        valid_input = True
    # Send a database to the scripts developer.
    elif options == "9" or options == "send":
        print("\nPlease pick a database you would like to email to the scripts developer so your database entries can possibly be added to the main database in a future update. The main database is downloaded automatically when the script is ran unless changed in config.py. The main database is recommended to be used when searching the database for the best results.")
        print("\nSorry but this feature is not available right now. Please manually email the database to brianleek2016@gmail.com. Thank you!")

        valid_input = True
    elif options == "10" or options == "stats":
        GetInformationFunctions.database_files()
        DatabaseFunctions.connect()

        DatabaseFunctions.database_stats()
